...
class ShopifyService
{
    ...
    /**
     * Update order metafields in Shopify
     */
    public function updateOrderMetafields(string $orderId, array $metafields): bool
    {
        $this->waitForShopifyRateLimit();

        // First, get existing metafields to merge
        $order = $this->getOrder($orderId);
        $existingMetafields = [];
        if (isset($order['metafields']) && is_array($order['metafields'])) {
            foreach ($order['metafields'] as $mf) {
                $existingMetafields[$mf['namespace'] . '.' . $mf['key']] = $mf;
            }
        }

        // Prepare metafields for update
        $metafieldsToUpdate = [];
        foreach ($metafields as $namespace => $fields) {
            if (is_array($fields)) {
                foreach ($fields as $key => $value) {
                    $fullKey = $namespace . '.' . $key;
                    $existing = $existingMetafields[$fullKey] ?? null;
                    ...
                    if ($existing && isset($existing['id'])) {
                        ...
                        $url = "https://{$this->store->shopify_store_url}/admin/api/2024-10/metafields/{$existing['id']}.json";
                        $this->waitForShopifyRateLimit();
                        $response = Http::withHeaders([
                            'X-Shopify-Access-Token' => $this->store->shopify_access_token,
                            'Content-Type' => 'application/json',
                        ])->put($url, ['metafield' => $metafieldData]);

                        if ($response->failed()) {
                            Log::error("Failed to update metafield {$namespace}.{$key}: HTTP {$response->status()}");
                            throw new \Exception("Failed to update metafield {$namespace}.{$key}");
                        }
                    } else {
                        // Create new metafield
                        $url = "https://{$this->store->shopify_store_url}/admin/api/2024-10/metafields.json";
                        $this->waitForShopifyRateLimit();
                        $response = Http::withHeaders([
                            'X-Shopify-Access-Token' => $this->store->shopify_access_token,
                            'Content-Type' => 'application/json',
                        ])->post($url, ['metafield' => $metafieldData]);

                        if ($response->failed()) {
                            Log::error("Failed to create metafield {$namespace}.{$key}: HTTP {$response->status()}");
                            throw new \Exception("Failed to create metafield {$namespace}.{$key}");
                        }
                    }
                }
            }
